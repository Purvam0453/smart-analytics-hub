...
async def upload_resume(
    ...
    # Log upload action
    save_log(username, "Resume Upload", filename)
    ...
    predicted_role = analysis.get("predicted_role", "General")
    resume_score = analysis.get("resume_score", 0.0)
    recommendations = analysis.get("recommendations", [])

    # Save to SQLite / DB model
    try:
        db_resume = Resume(
            user_id=user_id,
            filename=filename,
    ...
    # Sync to CSV for backwards compatibility
    try:
